  ...
  ({
    ...
    const typeElement = useComputed$(() => {
      ...
      const styleLinkDisabled =
        disabled && base === "Link" && option === undefined
      ...
      return {
        base,
        optionStyle: option
          ? "bg-black border-2 border-amber-500 text-amber-500 hover:bg-amber-500 hover:border-transparent hover:text-black"
          : "bg-amber-500 text-slate-900 font-bold border-amber-500 hover:bg-amber-300 hover-border-amber-300",
        styleLinkDisabled,
        styleButtonDisabled,
      };
    });
      ...
      <>
        {typeElement.value.base === "Button" && (
          <button
            ...
            disabled={disabled}
            class={[
              " md:w-auto px-8 py-2 border-2 rounded-md flex flex-row items-center justify-center gap-1",
              typeElement.value.optionStyle,
              `${typeElement.value.styleButtonDisabled}`,
              `${className}`,
            ]}
          >
            <Slot />
        ...
        {typeElement.value.base === "Link" && (
          <Link
            href={href}
            class={[
              "px-8 py-2 rounded-md duration-500 flex items-center justify-center ",
              typeElement.value.optionStyle,
              `${typeElement.value.styleLinkDisabled}`,
              `${className}`,
            ]}
          >
            <Slot />
